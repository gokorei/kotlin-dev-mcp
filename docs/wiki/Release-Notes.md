...
- **Jetpack Compose Lazy layout key auditing & mutated state collision detection in `ComposeAnalyzer`** — added K2 PSI AST inspection for `LazyColumn`, `LazyRow`, `LazyVerticalGrid`, and lazy layout DSL calls. Detects keys referencing variables mutated inside the layout body (preventing runtime `IllegalArgumentException: Key was already used` crashes), flags duplicate static literal keys and constant-key lambdas, de-duplicates missing-key advisories to at most once per container, and ignores `item` singletons.
...
- **Modern AGP 8/9 Android classpath resolution** — expanded `detectProjectClasspath` in `SnippetCompiler` to discover compiled Android variant outputs in `build/tmp/kotlin-classes/`, AGP 9 built-in kotlinc outputs (`build/intermediates/built_in_kotlinc/.../classes`), and packaged JARs in `build/intermediates/runtime_library_classes_jar/` and `build/intermediates/compile_app_classes_jar/`.
- **Workspace-scoped Compose anti-pattern analysis** — added `analyzeWorkspace` support to `ComposeAnalyzer` and `CodeAnalysisService`, allowing `kotlin_code_analyze(action = "compose", workspacePath = "...")` to scan all `.kt` files across a project without needing to paste file contents.
...
- **Project quality gates with Detekt, KtLint, and Kover** — configured project-level static analysis via Detekt with baseline, code style enforcement via KtLint and `.editorconfig`, test coverage verification with Kover (75% threshold), wired all into `./gradlew check` and updated CI workflow (GXY1RWKS).
- **Supply chain security & open source governance** — added `CODE_OF_CONDUCT.md` (Contributor Covenant 2.1), automated weekly Gradle and GitHub Actions dependency updates via `.github/dependabot.yml`, pinned all GitHub Actions workflows to immutable commit SHAs, and configured automated SAST CodeQL analysis (KQRND6QR).
...
- **Android runtime target resolver & synthesized ADB CLI commands** — added `kotlin_project_inspect(action="android_runtime_target")` in `ProjectService` and `AndroidManifestInspector` to statically resolve application ID, namespace, and launcher activity (`<activity>` / `<activity-alias>`) from `AndroidManifest.xml` and Gradle build scripts, synthesizing ready-to-run ADB launch (`am start`) and logcat filter commands (`adb logcat --pid=...`).
- **Core Android app audit engine & targeted category dispatcher** — added `kotlin_project_inspect(action="android_audit")` in `AndroidAppAuditor` supporting targeted audits across Compose performance (unstable standard collection parameters, missing Lazy item `key`s, `collectAsStateWithLifecycle`), dangerous runtime permissions (`POST_NOTIFICATIONS`, PhotoPicker recommendations), and R8 minification / ProGuard rules.
- **WorkManager & CoroutineWorker architecture analyzer** — added `kotlin_code_analyze(action="workmanager")` and `kotlin_library_analyze(action="workmanager")` via `WorkManagerAnalyzer` to statically inspect `CoroutineWorker` and `ListenableWorker` subclasses for `@HiltWorker` + `@AssistedInject` constructor wiring, Android 14+ foreground service types, and blocking I/O dispatcher safety (`withContext(Dispatchers.IO)`).
- **Modern Android & Jetpack Compose progressive steering** — updated `FrameworkFeatureCatalog` and `kotlin-mcp-guidance/SKILL.md` with modern Android best practices (Type-Safe Navigation Compose 2.8+, `rememberSaveable`, `SavedStateHandle`, `enableEdgeToEdge`, `PickVisualMedia`, and `POST_NOTIFICATIONS` runtime checks), strictly gated to Android environment profiles to prevent context bloat on backend and non-Android KMP projects.
