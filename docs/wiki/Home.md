...
The server is distributed as a self-contained JVM uber-JAR (`kotlin-mcp-1.0.0-all.jar`) that requires no Gradle daemons or external services. Kotlin type checking and PSI/AST analysis run through the embedded K2 compiler.
...
```json
{
  "mcpServers": {
    "kotlin-mcp": {
      "command": "java",
      "args": ["-jar", "/absolute/path/to/kotlin-mcp-1.0.0-all.jar"]
    }
  }
}
```
